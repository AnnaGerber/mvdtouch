...
<li><xsl:value-of select="@shortname"/>: <xsl:value-of select="@longname"/>
 Backup: <select class="backup">
<xsl:variable name="backup" select="@backup"/>
<xsl:for-each select = "//version" >
<option>
<xsl:if test="@shortname=$backup">
<xsl:attribute name="selected"/>
</xsl:if>
<xsl:value-of select = "@shortname" /></option>
